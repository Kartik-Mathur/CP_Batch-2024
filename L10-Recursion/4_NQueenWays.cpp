...
bitset<100> column, rightDia, leftDia;
int board[100][100] = {0};
int ans = 0;

void queenWays(int row, int n) {
	// base case
	if (row	== n) {
		ans++;
		for (int i = 0; i < n; ++i)
	...
	{
		if (!column[col] and !rightDia[row + col] and !leftDia[row - col + n - 1]) {
			column[col] = rightDia[row + col] =
			                  leftDia[row - col + n - 1] = board[row][col] = 1;
			queenWays(row + 1, n);
			column[col] = rightDia[row + col] =
			                  leftDia[row - col + n - 1] = board[row][col] = 0;
		}
	}
}
