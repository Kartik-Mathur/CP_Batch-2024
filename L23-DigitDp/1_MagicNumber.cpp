...
ll magicNumber(string &s, ll pos, ll tight, ll current_mod, bool st, bool even) {
	// base case
	if (pos == s.size()) {
		return dp[pos][current_mod][tight][st][even] = (current_mod == 0);
	}
	...
	{
		if (st == 0) {
			if (i == 0) // We are about a leading zero
				ans = (ans +
				       magicNumber(s, pos + 1, tight & (i == end), current_mod, 0, even) % mod ) % mod;
			else if (i != d)
				ans = (ans +
				       magicNumber(s, pos + 1, tight & (i == end), (current_mod * 10 + i) % m, 1, 1) % mod ) % mod;
		}
		else {
			if (i == d and even) {
